...
class Spaces
  ...
  def self.add(name:, description:, price:, owner_id:)
    connection = establish_connection()
    result = connection.exec("INSERT INTO spaces (name, description, price, owner_id) VALUES ('#{name}','#{description}','#{price}', '#{owner_id}') RETURNING id, name, description, price, owner_id;")
